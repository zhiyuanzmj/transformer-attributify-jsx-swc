...
import transformerAttributifyJsxSwc from '../src/index'
...
describe('transformerAttributifyJsxSwc', () => {
  const uno = createGenerator({
    presets: [presetUno(), presetAttributify()],
  })
  ...
  it('transform', async () => {
    ...
    expect(code.toString()).toMatchInlineSnapshot(`
      "<div h-full=\\"\\" text-center=\\"\\" flex=\\"\\" select-none=\\"\\" className={red ? 'text-red': 'text-green'}>
        ...
        <div components={<div ababsolute=\\"\\"obottom-5=\\"\\"/div>}></div>
        <h1 flflex=\\"\\"1</h1>
        <div {...{ flex }} />  
        <div {...{ onClick: () => { grid(); flex } }} flflex=\\"\\">
        <div {...true ? flex : props.grid } {...grid || ( block ) && $flex } />  
        <div {...[, flex, [flex], !flex, -flex, +flex, ~flex, \\"flex\\", \`flex\` ] } />  
      </div>"
    `)
